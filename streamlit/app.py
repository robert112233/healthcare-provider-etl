...
app_by_staff_department = """
    ...
    JOIN dim_departments ON dim_staff.department_id = dim_departments.department_id 
...
with col1:
    chart = alt.Chart(df_1).mark_bar().encode(
        x=alt.X('department_name:N', title="Department", axis=alt.Axis(labelAngle=-45)),
...
missed_apps_by_demographic = """SELECT
    ...
    FROM
        dim_patients
        JOIN fact_appointments ON dim_patients.patient_id = fact_appointments.patient_id
    WHERE
...
with col2:
    chart = alt.Chart(df_2).mark_bar().encode(
        x=alt.X('age_demographic:N', title="Demographic", axis=alt.Axis(labelAngle=-45)),
...
number_of_patients_reporting_flu_this_month = pd.read_sql_query(patients_reporting_flu_like_symptoms_this_month, olap_conn)['number_of_patients'].iloc[0]
number_of_patients_this_month = pd.read_sql_query(patients_this_month, olap_conn)['number_of_patients'].iloc[0]

percentage_of_patients_reporting_flu_this_month = (number_of_patients_reporting_flu_this_month / number_of_patients_this_month) * 100
...
app_data = """SELECT
    ...
    COALESCE(SUM(CASE WHEN appointment_status = 'attended' THEN 1 ELSE 0 END), 0) AS attended,
    COALESCE(SUM(CASE WHEN appointment_status IN ('missed', 'cancelled') THEN 1 ELSE 0 END), 0) AS not_attended
    FROM
        fact_appointments
    GROUP BY
...
percentage_of_apps_attended_2025 = round((attended_2025 / total_apps_not_upcoming_2025) * 100, 1)

total_apps_not_upcoming_2024 = attended_2024 + attended_2024
percentage_of_apps_attended_2024 = round((attended_2024 / total_apps_not_upcoming_2024) * 100, 1)

percentage_change = round((percentage_of_apps_attended_2025 - percentage_of_apps_attended_2024) / percentage_of_apps_attended_2024 * 100, 1)
